    ...
    "ref":  [
            {
                ...
                "prof_depth_score":prof_depth_score
            }
        ]
    "score": score
    "sub_query": [
...
[
     {
          ...
          "ref": [
               {
                    ...
                    "content": {
                         "title": "华为防火墙配置指南",
                         "url": "https://support.huawei.com/doc1",
                         "content": "",
                         "extracted_content": "USG6000系列支持ACL规则配置。",
                         "relevance_score": 0.92,
                         "prof_depth_score": 0.87
                    },
                    "extracted_content": "企业应部署下一代防火墙。",
                    "relevance_score": 0.85,
                    "prof_depth_score": 0.9
               }
          ],
          "score": [
               0.89,
          ...
          "sub_query": [
               {
                    "sub_query1": "华为USG6000是否支持IPv6 ACL？",
                    "ref": [
                         {
                              "title": "华为防火墙配置指南",
                              "url": "https://support.huawei.com/doc1",
...
class ReferenceDict(TypedDict):
    """
    一个用于存储引用信息的TypedDict类，用于结构化地存储和管理文档引用的相关数据。

    核心功能：
    - 存储引用的深度、标题、URL和内容
    - 管理提取的内容和相关性评分
    - 支持嵌套的引用结构（通过content字段）
    ...
    - title: str - 引用的标题
    - url: str - 引用的来源URL
    - content: ReferenceDict | str - 引用的原始内容，可以是字符串或嵌套的ReferenceDict
    - extracted_content: str - 从引用中提取的文本内容
    - relevance_score: float - 相关性评分（0-1之间）
    - prof_depth_score: float - 专业深度评分（0-1之间）
    - score: float - 综合评分（0-1之间）

    注意：
    - 所有评分字段应在0-1的范围内
    - content字段支持嵌套结构，可以包含另一个ReferenceDict实例
    """
    depth: int
    title: str
    url: str
    content: ReferenceDict | str
    extracted_content: str
    relevance_score: float
    prof_depth_score: float
    score: float
...
if __name__ == "__main__":
    # ReferenceItem → SubQueryItem → QueryResult
    # 示例输入
    # 叶子参考文献（content 为字符串）
    ref1: ReferenceItem = {
        "title": "华为防火墙配置指南",
    ...
    ref2: ReferenceItem = {
        "title": "网络安全白皮书",
        "url": "https://example.com/whitepaper",
        #    "content": ref1,  # 嵌套另一个 ReferenceItem（可选）
        "content": "",  # 嵌套另一个 ReferenceItem（可选）
        "extracted_content": "企业应部署下一代防火墙。",
        "relevance_score": 0.85,
        "prof_depth_score": 0.90,
    }

    # 子查询
    sub_q1: SubQueryItem = {"sub_query": "华为USG6000是否支持IPv6 ACL？", "ref": [ref1]}

    # 顶层结果
    result: QueryResult = {
        "query": "华为企业级防火墙有哪些高级功能？",
        "rewrite": "",
        "ref": [[ref2]],
        "score": [0.89, 0.95],
        "sub_query": [sub_q1],
    ...
    # 打印验证
    print("主问题:", result["query"])
    print("主得分:", result["score"])
    print("子问题数量:", len(result["sub_query"]))
    print("第一个子问题:", result["sub_query"][0]["sub_query"])
    print("----" * 30)
    print(result)
    #     HistoryMessage = list[QueryResult]
    #     history_message: HistoryMessage = [] # type: ignore

    #     history_message.append(result)
    #     history_message.append(result)
    print("----" * 30)
